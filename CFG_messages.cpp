...
namespace BPPGNSS {
	...
	CFG_PRT_UART::CFG_PRT_UART(void){
		...
		//bools for txReady
		for(int i = 0;i<9;i++) thres[i] = 0;
		for(int i = 0;i<5;i++) pin[i] = 0;
		pol = 0;
		en = 0;

		//bools for mode
		for(int i = 0;i<2;i++) nStopBits[i] = 0;
		for(int i = 0;i<3;i++) parity[i] = 0;
		for(int i = 0;i<2;i++) charLen[i] = 0;
		reserved1 = 0;
	...
	void CFG_PRT_UART::encodeMsg(uint8_t* buf){
		...
		//Encode txReady
		txReady = thres[8]<<15|thres[7]<<14|thres[6]<<13|thres[5]<<12|thres[4]<<11|thres[3]<<10|thres[2]<<9|thres[1]<<8|thres[0]<<7|pin[4]<<6|pin[3]<<5|pin[2]<<4|pin[1]<<3|pin[0]<<2|pol<<1|en;
		temp[0] = txReady;
		buf[NUM_HEADER_BYTES + 2] = temp[0];
		// +Praise Jesus+
		//Encode mode
